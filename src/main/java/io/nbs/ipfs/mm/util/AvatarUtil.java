...
public class AvatarUtil {
    ...
    public static Image getPeerAvatar(PeerInfo info) throws IOException {
        ...
        File infoFile = new File(DappCnsts.consturactPath(AvatarImageHandler.getAvatarProfileHome(),info.getAvatarName()));
    ...
    public static Image createOrLoadUserAvatar(String identify,boolean isHash,String suffix){
        ...
        if(isHash){
            avatarPath = DappCnsts.consturactPath(CUSTOM_AVATAR_CACHE_ROOT,identify+".png");
            ...
            try {
                if(temFile.exists()){
                    ...
                    new Thread(()->{
                        URL url = null;
                        try {
                            File temCacheFile = new File(DappCnsts.consturactPath(AVATAR_CACHE_ROOT,identify+suffix));
                            logger.info("avatar download:{}",identify);
                            url = new URL(Launcher.LaucherConfMapUtil.getIpfsAddressGateway()+identify);
