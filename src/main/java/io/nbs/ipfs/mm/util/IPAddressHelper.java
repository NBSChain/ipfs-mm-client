...
import net.nbsio.ipfs.vo.TaobaoResult;
...
public class IPAddressHelper {
    ...
    private IPAddressHelper() {
        HTML_TEMP = DappCnsts.consturactPath(Launcher.CURRENT_DIR,"TEMP","html");
        File dirFile = new File(HTML_TEMP);
        if(dirFile.exists())dirFile.delete();
        dirFile.mkdirs();
        CHECKED_URL = Launcher.LaucherConfMapUtil.getValue("nbs.server.ip.checked-url","http://www.net.cn/static/customercare/yourip.asp");
        CHECKED_LOCATION_URL = Launcher.LaucherConfMapUtil.getValue("nbs.server.ip.checked-locations-url","http://ip.taobao.com/service/getIpInfo.php?ip=");
    }
